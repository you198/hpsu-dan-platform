...
export interface BearingFaultState {
  type: string;
  intensity: number;
  target?: "inner" | "outer" | "rolling" | "compound";
}

export class Bearing6205 {
  readonly group = new THREE.Group();
  readonly outerRace: THREE.Mesh;
  readonly innerRace: THREE.Mesh;
  readonly balls: THREE.Mesh[] = [];
  readonly cage: THREE.Mesh;
  readonly defectSpot: THREE.Mesh;

  private ballOrbitGroup = new THREE.Group();
  private time = 0;
  private faultState: BearingFaultState = { type: "normal", intensity: 0 };

  constructor() {
    // --- Outer Race ---
    this.outerRace = new THREE.Mesh(
      new THREE.TorusGeometry(0.78, 0.08, 24, 64),
      createMetalMaterial({ color: 0x3f5364, emissive: 0x3f5364, emissiveIntensity: 0.03 })
    );
    ...
    // --- Inner Race ---
    this.innerRace = new THREE.Mesh(
      new THREE.TorusGeometry(0.38, 0.08, 24, 64),
      createPolishedMaterial({ color: 0x546879, emissive: 0x546879, emissiveIntensity: 0.03 })
    );
    ...
    this.cage = new THREE.Mesh(
      new THREE.TorusGeometry(0.56, 0.025, 12, 48),
    ...
    this.defectSpot = new THREE.Mesh(
  ...
  setFault(state: BearingFaultState) {
    ...
    switch (state.type) {
      ...
      case "inner_race_fault":
        ...
        this.defectSpot ).material as THREE.MeshStandardMaterial).emissiveIntensity = 1.2;
        break;
      case "outer_race_fault":
        applyFaultMaterial(this.outerRace, "outer_race_fault", 0.7);
        this.defectSpot ).material as THREE.MeshStandardMaterial).emissiveIntensity = 1.0;
        break;
      case "rolling_element_fault":
        this.balls[Math.floor(Math.random() * 9)] ).material as THREE.MeshStandardMaterial).emissiveIntensity = 0.8;
        this.defectSpot ).material as THREE.MeshStandardMaterial).emissiveIntensity = 1.0;
        break;
      case "compound_fault":
        applyFaultMaterial(this.innerRace, "compound_fault", 0.5);
        applyFaultMaterial(this.outerRace, "compound_fault", 0.5);
        this.balls[2] ).material as THREE.MeshStandardMaterial).emissiveIntensity = 0.6;
        this.defectSpot ).material as THREE.MeshStandardMaterial).emissiveIntensity = 1.0;
        break;
    }
  }
  ...
  update(deltaTime: number) {
    ...
    if (fault === "inner_race_fault") {
      ...
      this.defectSpot ).material as THREE.MeshStandardMaterial).emissiveIntensity = 0.7 + Math.sin(t * 8) * 0.45;
      ...
      this.defectSpot ).material as THREE.MeshStandardMaterial).emissiveIntensity = 0.7 + Math.sin(t * 5) * 0.45;
      ...
      this.defectSpot ).material as THREE.MeshStandardMaterial).emissiveIntensity = 0.7 + Math.sin(t * 7) * 0.5;
    } else if (fault === "compound_fault") {
      this.defectSpot.position.y = Math.sin(t * 0.6) * 0.4;
      this.defectSpot.position.z = Math.cos(t * 0.6) * 0.4;
      this.defectSpot ).material as THREE.MeshStandardMaterial).emissiveIntensity = 0.8 + Math.sin(t * 6) * 0.6;
    }
  }
}
