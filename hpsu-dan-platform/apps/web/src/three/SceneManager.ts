...
import { PartPicker, PART_LIBRARY } from "./controls/PartPicker";
...
export class SceneManager {
  ...
  readonly picker: PartPicker;
  readonly bearing: Bearing6205;

  private frame = 0;
  private startTime = performance.now();
  private faultKey = "normal";
  private twinTarget = "";
  ...
  constructor(opts: SceneOptions) {
    ...
    (container as any)._sceneObserver = ro;
  ...
  private buildExperimentPlatform(): Bearing6205 {
    ...
    for (const [bx, bz] of [[-4.8, -1.2], [-4.8, 1.2], [4.8, -1.2], [4.8, 1.2]]) {
      const hole = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({ color: 0x0a1420 })
      );
    ...
    for (let i = 0; i < 10; i++) {
      const fin = new THREE.Mesh(
        new THREE.BoxGeometry(0.06, 0.72, 2.34),
        new THREE.MeshStandardMaterial({ color: 0x0f1d27, metalness: 0.45, roughness: 0.55 })
      );
    ...
    const ctrl = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x10283a, emissive: 0x0b4d62, emissiveIntensity: 0.18, metalness: 0.3, roughness: 0.45 })
    );
    ...
    const cp = new THREE.Mesh(
      new THREE.CylinderGeometry(0.36, 0.36, 0.52, 32),
      new THREE.MeshStandardMaterial({ color: 0xdfe8ea, emissive: 0x1c3b48, emissiveIntensity: 0.08, metalness: 0.65, roughness: 0.18 })
    );
    ...
    const guard = new THREE.Mesh(
      new THREE.BoxGeometry(2.25, 1.35, 1.55),
      new THREE.MeshPhysicalMaterial({ color: 0x8fdfff, transparent: true, opacity: 0.16, roughness: 0.12, metalness: 0, transmission: 0.2 })
    );
    ...
    for (const cfg of sensorConfigs) {
      const body = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({ color: 0xc7d1d5, emissive: cfg.color, emissiveIntensity: 0.28, metalness: 0.55, roughness: 0.25 })
      );
    ...
    const gb = new THREE.Mesh(
      new THREE.CylinderGeometry(0.78, 0.78, 1.05, 36),
      new THREE.MeshStandardMaterial({ color: 0x80906f, metalness: 0.55, roughness: 0.34 })
    );
    ...
    const bg = new THREE.Mesh(
      new THREE.TorusGeometry(1.24, 0.035, 10, 48),
  ...
  setFault(faultKey: string, twinTarget?: string) {
  ...
  private getFaultTarget(): "inner" | "outer" | "rolling" | "compound" | undefined {
    const t = this.twinTarget;
    if (t.includes("inner")) return "inner";
    if (t.includes("outer")) return "outer";
    if (t.includes("rolling")) return "rolling";
  ...
  dispose() {
    ...
    if (el.parentElement) {
      ...
      delete (el.parentElement as any)._sceneObserver;
    }
  }
}
