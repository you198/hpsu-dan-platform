...
export type FaultFamily = 'normal' | 'inner_race_fault' | 'outer_race_fault' | 'rolling_element_fault' | 'compound_fault' | 'unknown'
...
export type DiagnosisState = {
  ...
  riskLevel: string
  twinTarget: string
  waveformMode: 'stable' | 'impact_periodic' | 'impact_fixed' | 'impact_modulated' | 'compound'
  spectrumMarkers: string[]
  waveformPoints: { x: number; y: number }[]
  spectrumPoints: { frequency: number; magnitude: number }[]
  suggestionZh: string
  suggestionEn: string
  engineMode: string
  modelVersion: string
...
function inferFaultFamily(prediction: DiagnosisPrediction): FaultFamily {
  const target = prediction.twin_target || ''
  const code = (prediction.code || prediction.label || '').toUpperCase()
  if (target.includes('inner')) return 'inner_race_fault'
  if (target.includes('outer')) return 'outer_race_fault'
  if (target.includes('rolling')) return 'rolling_element_fault'
  if (target.includes('compound')) return 'compound_fault'
  if (code === 'K001' || code === 'NC' || code === 'NORMAL') return 'normal'
  if (code.startsWith('KI')) return 'inner_race_fault'
  if (code.startsWith('KA')) return 'outer_race_fault'
  if (code.startsWith('KB')) return 'rolling_element_fault'
  return 'unknown'
}

function evidenceFor(faultKey: FaultFamily) {
  const table = {
    normal: {
      waveformMode: 'stable',
      spectrumMarkers: ['1X'],
...
export const useDiagnosisStore = defineStore('diagnosis', {
  ...
  actions: {
    applyResult(result: any) {
      const prediction = (result?.prediction || {}) as DiagnosisPrediction
    ...
    setFault(faultKey: FaultFamily) {
      const evidence = evidenceFor(faultKey)
      const labels: Record<FaultFamily, [string, string, string, string]> = {
